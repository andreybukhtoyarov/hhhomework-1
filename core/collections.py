...
def even(some_not_even_list: list) -> list:
...
def get_ages(some_years_of_birth: list) -> list:
...
def get_first_n_last(some_list: list) -> list:
...
def get_list_without_repetition(some_list: list) -> list:
...
def map_keys_and_values(some_keys: list, some_values: list) -> dict:
...
def count_symbols(some_string: str) -> dict:
    result = dict(zip(set(some_string), [0 for i in range(len(set(some_string)))]))
